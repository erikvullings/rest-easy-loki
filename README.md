...
-->
- `apiKey` maps `GET`, `POST`, `PUT/PATCH`, and `DELETE` to read, create, update, and delete key lists. An action with no configured keys remains public for CLI compatibility. A configured `publicRoutes` rule or whitelisted hostname bypasses key checks.
